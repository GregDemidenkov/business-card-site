...
const changeHeart = () => {
	document.querySelectorAll('.heart-to-favorite').forEach((item) => {
  		if (arrayOfFavorite.indexOf(item.id) !=-1) {
  			document.getElementById(item.id).style.background = "url('./public/img/store/Heart-in-cart-1.png')"
  		} else {
  			document.getElementById(item.id).style.background = "url('./public/img/store/Heart-in-cart.png')"
  		}
	})
}
...
const toFavorite = (clicked_id) => {
	if (arrayOfFavorite.indexOf(clicked_id) === -1) {
		...
		document.getElementById(clicked_id).style.background = "url('./public/img/store/Heart-in-cart-1.png')"
		saveInLocalF()
	} else {
		arrayOfFavorite.splice(arrayOfFavorite.indexOf(clicked_id),1)
		document.getElementById(clicked_id).style.background = "url('./public/img/store/Heart-in-cart.png')"
		saveInLocalF()
	}
}
...
const createHTMLforPost = (id) => {
	return ` 
		<div class="minProduct" id = "${id+'min'}">
			<img src="./public/img/store/image-${id[0]}.png" alt="">
...
const createHTMLforCart = (id) => {
	return ` 
		<div class="productInCart" id = "${id+'c'}">
			<img src="./public/img/store/image-${id}.png" alt="">
